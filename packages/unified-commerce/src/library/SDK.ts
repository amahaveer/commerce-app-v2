...
> {
	...
	private getDefaultAPIHeaders(customHeaderValue?: string) {
		...
		} = customValue ? { "custom-configuration": customValue } : {};
		return {
			locale: this.apiHubLocale(),
			currency: this._currency,
			"extension-version": this._extensionVersion ?? "",
			...customHeader,
		};
	}
	...
	async callAction<ReturnData>(options: {
		...
		const url = normaliseUrl(
			`${this._endpoint}/${options.actionName}${params}`
		);
		const action = () =>
			fetcher<ReturnData>(
				url,
	...
	page: PageApi = {
		getPage: async (options: {
			...
			const url = normaliseUrl(`${this._endpoint}/page${params}`);
		...
		getPreview: async (options: {
			...
			const url = normaliseUrl(
				`${this._endpoint}/preview${generateQueryString(query)}`
			);
		...
		getPages: async (
			...
			const url = normaliseUrl(
				`${this._endpoint}/structure${generateQueryString(query)}`
			);
