...
import { getTemplateController } from '../../src/controllers/templateController';
...
describe('getTemplateController', () => {
  let mockRequest: any;
  ...
  it('should return 200 and template that matches id successfully', async () => {
    const paramsDraft = { id: 'homeNavigation' };
    ...
    await getTemplateController(
      mockRequest as Request,
      mockResponse as Response,
    );

    expect(getTemplate).toHaveBeenCalledWith(paramsDraft.id);
  ...
  it('should return 500 when an internal error occurs', async () => {
    ...
    (getTemplate as jest.Mock).mockImplementation(() => {
      throw error;
    });
