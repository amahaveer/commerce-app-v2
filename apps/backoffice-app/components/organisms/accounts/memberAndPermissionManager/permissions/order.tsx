...
import { orderPermissionFields } from './def';
...
const OrderPermissions = (props: IPropType) => {
	...
	return (
		<AccordianUnControlled title={"common.orders"} className='border-0' labelClass='text-[1.25rem]'>
			<Box className="grid gap-0 px-6" sx={{ gridTemplateColumns: 'repeat(2, 1fr)' }}>
				{orderPermissionFields.map((item: any, index) => (
