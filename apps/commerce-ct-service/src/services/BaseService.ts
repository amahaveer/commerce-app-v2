...
const parseLocale = (locale: string, currency?: string): ParsedLocale => {
  ...
  if (territory === undefined) {
    if (language && language in languageToTerritory) {
      if (language && language in languageToTerritory) {
        territory =
          languageToTerritory[language as keyof typeof languageToTerritory];
      }
    } else {
      territory = language ? language.toUpperCase() : '';
    }
  }
  ...
  if (!currency) {
    ...
    if (territory && territory in territoryToCurrency) {
      currency =
        territoryToCurrency[territory as keyof typeof territoryToCurrency];
    }
    ...
    if (modifier !== undefined) {
      if (modifier in modifierToCurrency) {
        if (modifier in modifierToCurrency) {
          currency =
            modifierToCurrency[modifier as keyof typeof modifierToCurrency];
        }
      } else {
...
const associateRolesCache: {
  [projectKey: string]: {
    associateRoles: CommercetoolsAssociateRole[];
    expiryTime: number;
  };
} = {};
...
const pickCommercetoolsLanguage = (
  parsedLocale: ParsedLocale,
  availableLanguages: string[],
): string | undefined => {
  const candidates = [
    `${parsedLocale.language}-${parsedLocale.territory}`,
  ...
  const foundPrefix = availableLanguages.find((option) =>
...
export abstract class BaseService {
  protected apiRoot: ApiRoot | null = null;
  ...
  constructor(
    ...
    this.defaultLocale = commercetoolsFrontendContext.defaultLocale;
    this.defaultCurrency = defaultCurrency;

    this.locale = locale !== null ? locale : this.defaultLocale;
    this.currency = currency !== null ? currency : this.defaultCurrency;

    this.clientSettings = getConfig(commercetoolsFrontendContext);
    ...
    this.defaultAssociateRoleKeys = this.clientSettings
      ?.defaultAssociateRoleKeys || ['admin'];
    this.productIdField = this.clientSettings?.productIdField || 'id';
    this.categoryIdField = this.clientSettings?.categoryIdField || 'key';
    const storedToken = clientTokensStored.get(this.getClientHashKey());
  ...
  async setSessionCheckoutSessionToken(
    cartId: string,
    token: Token,
  ): Promise<void> {
  ...
  async getSessionCheckoutSessionToken(
    cartId: string,
  ): Promise<Token | undefined> {
    const checkoutHashKey = await this.getCheckoutHashKey(cartId);
    return (
      this.sessionData?.checkoutSessionToken?.[checkoutHashKey] ?? undefined
    );
  }
  ...
  protected async getCommercetoolsLocal(
    ...
    if (clientType === ClientType.BO) {
      ...
      const currency = project.currencies[0];
  ...
  protected async generateCheckoutSessionToken(cartId: string) {
    const checkoutSessionToken =
      await this.getSessionCheckoutSessionToken(cartId);
    ...
    if (checkoutSessionToken) {
      try {
        return await this.refreshCheckoutSessionToken(
          cartId,
  ...
  private async refreshCheckoutSessionToken(
    cartId: string,
  ...
  private async fetchCheckoutSessionToken(
    cartId: string,
    url: string,
    body: string,
  ) {
    ...
    const token: Token = {
      token: response?.id,
      expirationTime: response?.expiryAt
        ? new Date(response?.expiryAt).getTime()
        : undefined,
    };
  ...
  protected associateRequestBuilder(
    accountId: string,
  ): ByProjectKeyAsAssociateByAssociateIdRequestBuilder {
    ...
    return (() => {
      const get = (): TokenStore => {
        const tokenStore: TokenStore = {
          ...
          expirationTime: this.token?.expirationTime ?? 0,
  ...
  private getClientHashKey(): string {
    ...
    this.clientHashKey = crypto
      .createHash('md5')
      .update(
        this.clientSettings.clientId +
          this.clientSettings.clientSecret +
          this.clientSettings.projectKey,
      )
      .digest('hex');
