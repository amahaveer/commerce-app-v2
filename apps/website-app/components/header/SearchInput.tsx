...
const SearchInput = () => {
  ...
  const locale = getLocale(currentLocale as string);
    const slug = 'query';
  router.replace(`/${currentLocale}/search/${slug}/${searchKeyword}`);
  }

  return (
    <div className="flex w-80 border border-[#2E2E2E] rounded-md overflow-hidden">
      ...
      <input
        ...
        aria-label="Search"
        className="w-full h-[40px] px-3 bg-white text-sm focus:outline-none"
        type="search"
